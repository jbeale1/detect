...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    while(1){
        ...
        // printf("%d\n", nboxes);  // DEBUG: how many total, probably overlapping boxes before nms() step

        //if (nms) do_nms_obj(boxes, probs, l.w*l.h*l.n, l.classes, nms);
        if (nms) do_nms_sort(dets, nboxes, l.classes, nms);

        // printf("Image size: (%d x %d)\n",im.w, im.h);  // DEBUG report image size
        int found = 0;  // total # of items found in entire image
	for(int i=0;i<nboxes;i++){
           int hit=0;  // items found in this particular box

           for(j = 0; j < l.classes; ++j){
             if (dets[i].prob[j] > thresh){
                 printf("%s: %.1f%% ", names[j], dets[i].prob[j]*100);
                 hit++;
                 found++;
             }
           }
           if (hit > 0) {
             int margin = 10;  // extra pixels for crop around bounding box
             int x1 = im.w * (dets[i].bbox.x-(dets[i].bbox.w / 2));
             int y1 = im.h * (dets[i].bbox.y-(dets[i].bbox.h / 2));
             int x2 = x1 + im.w*dets[i].bbox.w;
             int y2 = y1 + im.h*dets[i].bbox.h;
             x1 = fmax(x1-margin,0); // upper-left corner
             y1 = fmax(y1-margin,0);
             x2 = fmin(x2+margin, im.w-1); // lower-right corner
             y2 = fmin(y2+margin, im.h-1);
             int cw = x2-x1;  // crop width
             int ch = y2-y1;  // crop height
             printf("%d: Crop: %dx%d+%d+%d\n", found,cw,ch,x1,y1); 

             // imc = crop(im,crop_spec);
             // save_image(imc, crop_filename);

             // printf("Box %d at (x,y)=(%5.3f,%5.3f) with (w,h)=(%5.3f,%5.3f)\n", i, dets[i].bbox.x, dets[i].bbox.y, dets[i].bbox.w, dets[i].bbox.h); 
           }
	}
        ...
        if(outfile){
           ...
           // save_image(im, "predictions"); // save predicted image with boxes & labels
#ifdef OPENCV
